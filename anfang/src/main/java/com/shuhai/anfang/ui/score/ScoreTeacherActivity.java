...
import com.shuhai.anfang.bean.BeanExamTeacher;
import com.shuhai.anfang.bean.BeanScoreTeacher;
import com.shuhai.anfang.bean.ExamType;
...
public class ScoreTeacherActivity extends BaseListActivity {
    ...
    private void initDate() {
        ...
        spnExams.setOnItemSelectedListener(new MaterialSpinner.OnItemSelectedListener<BeanExamTeacher>() {

            @Override
            public void onItemSelected(MaterialSpinner view, int position, long id, BeanExamTeacher item) {
                flTransparent.setVisibility(View.GONE);
                initTitleExam(item);
                getFirstPageData();
            }
    ...
    public void initTitleExam(BeanExamTeacher item) {
        if (item == null) {
            setTitle(R.string.home_score);
            return;
        }
            ...
            public void onResponse(VolleyHttpResult volleyHttpResult) {
                ...
                switch (volleyHttpResult.getStatus()) {
                    case HttpAction.SUCCESS:
                        try {
                            Gson gson = new Gson();
                            ArrayList<BeanExamTeacher> listExams = gson.fromJson(volleyHttpResult.getData().toString(), new TypeToken<List<BeanExamTeacher>>() {
                            }.getType());
                            if (listExams.size() > 0) {
                                spnExams.setEnabled(true);
                                spnExams.setItems(listExams);
                                initTitleExam(listExams.get(0));
                                getFirstPageData();
                            } else {
                                spnExams.setText("暂无考试");
                                spnExams.setEnabled(false);
                                initTitleExam(null);
                            ...
                            Toast.makeText(ScoreTeacherActivity.this, ex.getMessage(), Toast.LENGTH_SHORT).show();
                            Log.i(TAG, "onResponse: exam name " + ex.getMessage());
                        }
                        break;
                    default:
                        spnExams.setItems(listExams);
                        Toast.makeText(ScoreTeacherActivity.this, "获取考试名称失败", Toast.LENGTH_SHORT).show();
                        break;
                }
            }
    ...
    private void getScoreList() {
        ...
        BeanExamTeacher exam = null;
        try {
            exam = (BeanExamTeacher) spnExams.getSelectedItem();
        } catch (Exception ex) {
            exam = null;
        }
        if (exam == null) {
                    ...
                    public void onResponse(VolleyHttpResult httpResult) {
                        ...
                        switch (httpResult.getStatus()) {
                            case HttpAction.SUCCESS:
                                try {
                                    //成绩
                                    List<BeanScoreTeacher> datas = new ArrayList<BeanScoreTeacher>();
                                    //头部课程
                                    ...
                                    if (type.equals(ExamType.ALL.toString())) {
                                        ...
                                        for (int i = 0; i < students.length(); i++) {
                                            ...
                                            BeanScoreTeacher score = new BeanScoreTeacher();
                                    ...
                                    for (int i = 0; i < strCourse.size(); i++) {
                                        TextView title = new TextView(ScoreTeacherActivity.this);
                                    ...
                                    Toast.makeText(ScoreTeacherActivity.this, "获取成绩失败", Toast.LENGTH_SHORT).show();
                                }
                                break;
                            default:
                                Toast.makeText(ScoreTeacherActivity.this, httpResult.getInfo(), Toast.LENGTH_SHORT).show();
                                break;
                        }
